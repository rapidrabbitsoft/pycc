...
from .utils import print_header, print_summary
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="pycc",
        description=(
            "Python Code Checker - A comprehensive tool for running various "
...
Examples:
  ...
  pycc --check black flake8     # Run specific checkers
    ...
    action_group.add_argument(
        "--all", "-a", action="store_true", help="Run all available checks"
    )
    action_group.add_argument(
        "--check",
        "-c",
    ...
    action_group.add_argument(
        "--list", "-l", action="store_true", help="List all available checkers"
    )
    action_group.add_argument(
        "--generate-config",
        "-g",
...
def main():
    ...
    if args.all:
        ...
        if selected_categories:
            ...
            parser.error(
                "No checkers selected. Use --all, --check, or specify categories."
            )

    # Validate project path
    if not args.project_path.exists():
        print(
            f"Error: Project path '{args.project_path}' does not exist.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not args.project_path.is_dir():
        print(
            f"Error: Project path '{args.project_path}' is not a directory.",
            file=sys.stderr,
        )
        sys.exit(1)
